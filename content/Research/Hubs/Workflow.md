...
A topic claim is either sourced or marked as a question. `verification:` records what was checked (`abstract_checked`, `full_text_checked`, `educational_checked` and the official page or document checks), and `reading_status:` stays `queued` until the source has actually been read. Downloaded does not mean read.
