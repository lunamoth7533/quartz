...
**Four inferential steps.** Genetic inference climbs a ladder from variant to use: association (a variant correlates with a trait, marking a region), fine-mapping (which variants are credible; it narrows candidates), mechanism (what the variant does, identified by functional work) and prediction (how well it classifies; scores aggregate variants for this), with uses such as risk stratification framed by guideline bodies as areas under investigation. Each step requires its own evidence, and most reporting collapses them: the common error is citing a rung-two finding in rung-four language - prediction language attached to association evidence. [[P29844615 Schaid 2018 Fine-mapping#^p29844615-problem|Fine-mapping]] [[P28686856 Visscher 2017 10 years of GWAS discovery#^p28686856-translation|10 years of GWAS discovery]] [[F68 NHGRI Polygenic risk scores#^f68-use|Polygenic risk scores]]
...
**A score is a weighted sum with a birthplace.** A polygenic score is built by weighting many variant associations from a discovery sample and summing them in a target sample, and its performance depends on discovery sample size, the genetic architecture of the trait and the similarity between samples. The score applied to a new person imports the discovery sample's weights, its ancestry and its ascertainment; portability degrades when any of the three shift, which is why score performance drops across ancestry groups and why calibration in the target population is part of responsible use rather than an optional refinement. [[F68 NHGRI Polygenic risk scores#^f68-definition|Polygenic risk scores]] [[P28686856 Visscher 2017 10 years of GWAS discovery#^p28686856-prs|10 years of GWAS discovery]] Because variant frequencies and linkage patterns differ across ancestries, a score validated in one population cannot be assumed to work in another; the official guidance states this limitation explicitly. [[F68 NHGRI Polygenic risk scores#^f68-limits|Polygenic risk scores]]
