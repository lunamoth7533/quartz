---
note_type: source
source_kind: paper
title: "Cognitive-behavioural interventions for attention deficit hyperactivity disorder (ADHD) in adults."
authors: "Lopez PL, Torrente FM, Ciapponi A, Lischinsky AG, Cetkovich-Bakmas M, Rojas JI, Romano M, Manes FF."
year: 2018
journal: "The Cochrane database of systematic reviews"
publisher: ""
condition: [adhd]
domain: [psychology]
study_type: "Cochrane systematic review"
population: "14 randomized trials with 700 adults with ADHD; 13 trials from the northern hemisphere and one from Australia."
doi: "10.1002/14651858.cd010840.pub2"
pmid: "29566425"
pmcid: "PMC6494390"
source_url: "https://doi.org/10.1002/14651858.cd010840.pub2"
first_online: "2018-03-23"
access_level: open-access-online
verification: full_text_checked
verified_on: 2026-09-25
reading_status: queued
queue_tier: core
pdf: ""
cssclasses: [research-source]
tags: [research/source, research/condition/adhd, research/domain/psychology]
---

# Cognitive-behavioural interventions for attention deficit hyperactivity disorder (ADHD) in adults.

**Authors.** Lopez PL, Torrente FM, et al. (8 authors) · [PubMed record](https://pubmed.ncbi.nlm.nih.gov/29566425/)

**Published.** 2018 in The Cochrane database of systematic reviews.

**Identifiers.** DOI [10.1002/14651858.cd010840.pub2](https://doi.org/10.1002/14651858.cd010840.pub2) · PMID [29566425](https://pubmed.ncbi.nlm.nih.gov/29566425/) · PMCID [PMC6494390](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6494390/)

**Verification.** Abstract checked 2026-09-24; full text retrieved and checked 2026-09-25 (`receipts/source-checks/P29566425.txt`).

## Reported findings (checked abstract and full text)

Adding CBT to medication beat medication alone on clinician-rated and self-rated core symptoms, and CBT also beat a waiting list on self-reported symptoms.
^p29566425-symptoms

Certainty was low or very low for most comparisons: trials were small and outcome measures heterogeneous. Estimates also depended on the comparator - large against waiting list, not statistically significant against supportive psychotherapy.
^p29566425-certainty

Some secondary outcomes improved, including self-reported depression and anxiety, while quality-of-life results varied.
^p29566425-secondary

## Scope as reported

Cochrane review of randomized trials in adults; adverse events were rare.
^p29566425-limit

## Analyst cautions (AI synthesis)

These paragraphs are this library's own methodological appraisal, not statements from the paper.

Outcome depends on the rater: clinician-reported and self-reported results differed, so a benefit claim should name who rated it.
^p29566425-caution-rater

## Used by

- [[Reading a study and matching populations]]
- [[CBT and its evidence base]]
- [[ADHD diagnosis and measurement]]
- [[CBT for adult ADHD]]

## Working notes

- Reading status: `queued` (queued, not read). Access: `abstract-only`. Queue tier: `core`.
